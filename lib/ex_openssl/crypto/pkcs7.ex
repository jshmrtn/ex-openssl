defmodule ExOpenssl.Crypto.PKCS7 do
  ...
  """
  @spec encrypt(certs :: [X509.certificate], input :: binary, cipher :: Cipher.cipher, flags :: flags)
  ...
  """
  @spec sign(signcert :: X509.certificate, pkey :: PKey.key, certs :: [X509.certificate], input :: binary,
    flags :: flags)
    :: {:ok, pkcs7} | {:error, [Error.t]}
  def sign(signcert, pkey, certs \\ [], input, flags \\ [:stream])
  def sign(signcert, pkey, certs, input, flags)
    when is_reference(signcert) and is_reference(pkey) and is_list(certs) and is_binary(input) and is_list(flags)
    and length(flags) > 0,
  ...
  """
  @spec verify(pkcs7 :: pkcs7, certs :: [X509.certificate], store :: [X509.certificate], indata :: nil | binary,
    flags :: flags)
  ...
  def verify(pkcs7, certs, store, indata, flags)
    when is_reference(pkcs7) and is_list(certs) and is_list(store) and (is_binary(indata) or is_nil(indata))
    and is_list(flags) and length(flags) > 0,
